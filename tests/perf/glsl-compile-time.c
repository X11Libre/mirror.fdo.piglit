...
piglit_display(void)
{
	int64_t t0, t1;
	int i;
	int64_t fixed_func_time = 0, glsl_time_1 = 0, glsl_time_2 = 0;
	...
	printf("Time to draw fixed-function points: %"PRId64" ms\n", fixed_func_time);
	printf("Time to draw 1st GLSL shader points: %"PRId64" ms\n", glsl_time_1);
	printf("Time to draw 2nd GLSL shader points: %"PRId64" ms\n", glsl_time_2);
...
create_shader_program(const char *fs_code,
                      int64_t *compile_time, int64_t *link_time)
{
	GLuint fragShader;
	GLuint vertShader;
	GLuint program;
	...
	vertShader = piglit_compile_shader_text(GL_VERTEX_SHADER, VS_code);

	t0 = piglit_time_get_nano();
	fragShader = piglit_compile_shader_text(GL_FRAGMENT_SHADER, fs_code);
	t1 = piglit_time_get_nano();
	program = piglit_link_simple_program(vertShader, fragShader);
	t2 = piglit_time_get_nano();
...
Init(void)
{
	GLuint vbo;
	int i;
	int64_t compile_time, link_time, total_compile_time, total_link_time;
	...
	printf("Total glCompileShader() time: %"PRId64" ns\n", total_compile_time);
	printf("Total glLinkProgram() time: %"PRId64" ns\n", total_link_time);
